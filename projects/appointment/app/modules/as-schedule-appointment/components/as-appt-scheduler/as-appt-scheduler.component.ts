...
@Component({
  selector: 'as-appt-scheduler',
  templateUrl: './as-appt-scheduler.component.html',
  styleUrls: ['./as-appt-scheduler.component.scss']
})
export class AsApptSchedulerComponent implements OnInit, OnDestroy {
  ...
  getAppointmentDataForEdit() {
    this.dataService.getappointmentDataForEdit().pipe(
      switchMap((modifyAppointmentData: ModifyAppointmentData) => {
        this.isEditFromSummaryPage = true;
        this.hasChangedLocation = true;
        this.appointmentData = modifyAppointmentData;
        this.editAppointment(modifyAppointmentData);
        return this.googleMapService.getLatLngByZipcode(modifyAppointmentData.pscDetails.zip);
      })
  ...
  getAppointmentDataForLocation() {
    this.dataService.getReasonDataFindLocation().pipe(
      ...
      switchMap((data: any) => {
        ...
        return this.googleMapService.getLatLngByZipcode(this.selectedZipcode);
      })
    ).subscribe((coords: any) => {
      const userLocation: UserLocation = {
        latitude: coords.latitude,
  ...
  getLocationDataForEdit() {
    this.dataService.getfindLocationSelectedLocation().pipe(
      switchMap((locationFinderData: PscLocationAvailability) => {
        this.isEditFromSummaryPage = true;
        this.hasChangedLocation = true;
        this.selectedZipcode = locationFinderData.zip;
        return this.googleMapService.getLatLngByZipcode(locationFinderData.zip);
      })
    ).subscribe((coords: any) => {
      const userLocation: UserLocation = {
        latitude: coords.latitude,
  ...
  locationOnchange() {
    this.userLocation$.pipe(
      mergeMap((location: UserLocation) => {
        return this.googleMapService.getZipcodeByLatLong(location.latitude, location.longitude);
      }),
      mergeMap((address) => {
